...
class MDEMADV2OptimizedStrategy(Strategy):
    """
    ...
    def __init__(
        ...
        be_trigger_percent: float = 0.015, # 1.5% profit triggers BE
        be_buffer_percent: float = 0.002,  # 0.2% buffer
        ...
        self.position_max_pnl_percent = 0.0
    ...
    def on_bar(self, bar: Dict[str, Any]):
        ...
        # --- Stop / Break-Even / Trailing Stop Logic ---
        current_pos = self.broker.get_position(symbol) if self.broker else None
        
        if current_pos is None or abs(current_pos.size) < 1e-6:
            self.position_max_pnl_percent = 0.0
            self.be_activated = False
            self.trailing_stop_activated = False
        else:
            entry_price = current_pos.entry_price
            pnl_percent = (float(close) - entry_price) / entry_price if current_pos.size > 0 else (entry_price - float(close)) / entry_price
            
            if pnl_percent > self.position_max_pnl_percent:
                self.position_max_pnl_percent = pnl_percent
                
            # Check Trailing Stop
            if self.position_max_pnl_percent >= self.trailing_stop_percent:
                self.trailing_stop_activated = True
                
            if self.trailing_stop_activated:
                # If we drop more than trailing_stop_percent from the peak, close
                if self.position_max_pnl_percent - pnl_percent >= self.trailing_stop_percent / 2: # trailing stop distance
                    target_weight = 0.0
                    
            # Check Break-Even
            elif self.position_max_pnl_percent >= self.be_trigger_percent:
                self.be_activated = True
                
            if self.be_activated and not self.trailing_stop_activated:
                if pnl_percent <= self.be_buffer_percent:
                    target_weight = 0.0
