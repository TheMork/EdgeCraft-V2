...
export default function MetricsCard({ metrics }: MetricsCardProps) {
  ...
  return (
    <Card className="border-primary/50">
      <CardHeader>
        <CardTitle className="uppercase tracking-widest text-primary">Backtest Metrics</CardTitle>
      </CardHeader>
      <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div>
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Total Return</div>
            <div className={`text-2xl font-bold font-mono ${(metrics.total_return || 0) >= 0 ? 'text-bullish text-glow' : 'text-bearish text-glow'}`}>
                {formatPercent(metrics.total_return)}
            </div>
        </div>
        <div>
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Sharpe Ratio</div>
            <div className="text-2xl font-bold font-mono text-primary">{formatNumber(metrics.sharpe_ratio)}</div>
        </div>
        <div>
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Sortino Ratio</div>
            <div className="text-2xl font-bold font-mono text-primary">{formatNumber(metrics.sortino_ratio)}</div>
        </div>
        <div>
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Max Drawdown</div>
            <div className="text-2xl font-bold font-mono text-bearish text-glow">{formatPercent(metrics.max_drawdown)}</div>
        ...
        <div>
            ...
            <div className="text-2xl font-bold font-mono text-primary">${formatNumber(metrics.final_equity)}</div>
        </div>
        <div>
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Total Trades</div>
            <div className="text-2xl font-bold font-mono text-primary">{formatNumber(metrics.total_trades, 0)}</div>
        </div>
      </CardContent>
    </Card>
  );
