...
if [ "${EDGECRAFT_SKIP_DOCKER:-0}" = "1" ]; then
    echo "Skipping QuestDB startup because EDGECRAFT_SKIP_DOCKER=1."
elif command -v docker >/dev/null 2>&1; then
    if docker compose up -d || docker-compose up -d; then
        echo "QuestDB started via docker compose."
    elif command -v sudo >/dev/null 2>&1; then
        echo "Docker requires elevated privileges. Trying sudo..."
        if sudo docker compose up -d || sudo docker-compose up -d; then
            echo "QuestDB started via sudo docker compose."
        else
            echo "Warning: Could not start QuestDB via docker. Start it manually and continue."
        fi
    else
        echo "Warning: Docker is available but compose start failed and sudo is unavailable."
    fi
else
    echo "Docker is not available. QuestDB was not started by this script."
fi
...
pkill -f "$ROOT_DIR/venv/bin/python -c from multiprocessing.spawn import spawn_main" || true
pkill -f "$ROOT_DIR/venv/bin/python -c from multiprocessing.resource_tracker import main" || true
